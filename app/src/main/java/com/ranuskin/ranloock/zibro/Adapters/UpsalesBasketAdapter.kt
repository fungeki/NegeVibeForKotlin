...
class UpsalesBasketAdapter(private val mContext: Context, val fragment: UpsalesSelectFragment, var upsaleList: MutableList<UpsalesObj>): RecyclerView.Adapter<UpsalesBasketViewHolder>(){
    ...
    override fun getItemCount(): Int {
        ...
        return upsaleList.size
    }

    override fun onBindViewHolder(p0: UpsalesBasketViewHolder, p1: Int) {
        val itemView = p0.itemView
        val model = upsaleList[p1]
        itemView.row_upsales_add_item_plus_title_textview.text = model.titleStr
        val priceOnApp = removeFractionIfNeeded(model.priceOnApp)
        val priceOnEvent = removeFractionIfNeeded(model.priceOnEvent)
        itemView.row_upsales_price_amount_on_event_textview.text = priceOnEvent
        itemView.row_upsales_price_on_zibro_actual_price_textview.text = priceOnApp

        itemView.row_upsales_add_delete_button.setOnClickListener {
            upsalesDelegate.removeItem(p1)
        }
    }

    private fun removeFractionIfNeeded(variable: Double): String{
        if ((variable % 1.0) == 0.0 ){
            return variable.toInt().toString()
        }
